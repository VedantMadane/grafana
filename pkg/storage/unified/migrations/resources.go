...
func registerMigrations(ctx context.Context,
	...
	for _, def := range registry.All() {
		...
		if shouldAutoMigrate(ctx, def, cfg, sqlStore) {
			registerMigration(mg, migrator, tableLocker, registry, client, def, WithAutoMigrate(cfg))
			continue
		}
		...
		registerMigration(mg, migrator, tableLocker, registry, client, def)
	}
	return nil
}
...
func registerMigration(mg *sqlstoremigrator.Migrator,
	...
	migration := NewResourceMigration(migrator, tableLocker, registry, def.GetGroupResources(), def.MigrationID, def.RenameTables, validators, opts...)
