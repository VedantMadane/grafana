...
func ProvideUnifiedStorageMigrationService(
	...
	return &UnifiedStorageMigrationServiceImpl{
		migrator:    migrator,
		tableLocker: &legacyTableLocker{sql: sql},
		cfg:         cfg,
		sqlStore:    sqlStore,
		kv:          kv,
		client:      client,
...
func RegisterMigrations(
	...
	if maxOpenConns < 8 {
		...
		// - 1 for the migration session (sess)
		// - 1 for gRPC connections
		// - 1 for the READ lock (MySQL) or SHARE lock session (Postgres)
		// - N for per-table RENAME connections on MySQL
		// - 1 for processlist polling during rename
